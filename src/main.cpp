...
static esp_err_t root_handler(httpd_req_t *req) {
    ...
    if (!s_ap_mode) {
        ...
        httpd_resp_send(req,
            ...
            "*{margin:0;padding:0;box-sizing:border-box}"
            "body{font-family:-apple-system,'Segoe UI',sans-serif;min-height:100vh;background:linear-gradient(135deg,#0a0e1a 0%%,#12102a 50%%,#0a1520 100%%);color:#e8eaf6}"
            ".tb{position:fixed;top:0;left:0;right:0;height:56px;background:rgba(12,14,30,.92);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-bottom:1px solid rgba(99,102,241,.15);display:flex;align-items:center;padding:0 16px;z-index:100}"
            ".hm{width:40px;height:40px;border:none;background:linear-gradient(135deg,rgba(99,102,241,.15),rgba(139,92,246,.1));border:1px solid rgba(99,102,241,.2);border-radius:10px;color:#c4b5fd;font-size:1.2rem;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s}"
            ".hm:active{background:linear-gradient(135deg,rgba(99,102,241,.25),rgba(139,92,246,.2));transform:scale(.95)}"
            ".ti{flex:1;text-align:center;font-size:.95rem;font-weight:700;color:#e0e7ff}"
            ".ti small{display:block;font-size:.5rem;color:rgba(196,181,253,.5);font-weight:400;margin-top:1px}"
            ".ov{position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:200;display:none}"
            ".ov.on{display:block}"
            ".dr{position:fixed;top:0;left:0;width:280px;height:100%%;background:linear-gradient(180deg,#0f0d2a 0%%,#13112c 100%%);z-index:300;transform:translateX(-100%%);transition:transform .3s cubic-bezier(.4,0,.2,1);display:flex;flex-direction:column;border-right:1px solid rgba(99,102,241,.12)}"
            ".ov.on+.dr,.dr.open{transform:translateX(0)}"
            ".dh{padding:24px 18px 16px;border-bottom:1px solid rgba(99,102,241,.1)}"
            ".dh h2{font-size:1.05rem;font-weight:700;color:#e0e7ff}"
            ".dh p{font-size:.6rem;color:rgba(196,181,253,.4);margin-top:4px;display:flex;align-items:center;gap:6px}"
            ".dn{flex:1;padding:10px 0}"
            ".di{display:flex;align-items:center;gap:14px;padding:13px 18px;text-decoration:none;color:rgba(196,181,253,.45);font-size:.85rem;font-weight:500;border-radius:0 24px 24px 0;margin-right:12px;transition:all .2s}"
            ".di:active{background:rgba(99,102,241,.08);color:#c4b5fd}"
            ".di.act{color:#818cf8;background:linear-gradient(90deg,rgba(99,102,241,.12),rgba(99,102,241,.03));border-left:3px solid #6366f1}"
            ".di .ic{font-size:1.15rem;width:26px;text-align:center}"
            ".df{padding:18px;border-top:1px solid rgba(99,102,241,.08)}"
            ".df p{font-size:.5rem;color:rgba(196,181,253,.2);text-align:center}"
            ".mn{padding:16px;padding-top:72px}"
            ".g{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:12px 0}"
            ".bx{background:linear-gradient(135deg,rgba(99,102,241,.06),rgba(139,92,246,.04));border:1px solid rgba(99,102,241,.1);border-radius:14px;padding:14px;text-align:center;transition:all .3s}"
            ".bx:active{transform:scale(.97);border-color:rgba(99,102,241,.2)}"
            ".bx .v{font-size:1.5rem;font-weight:800;background:linear-gradient(135deg,#e0e7ff,#c4b5fd);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}"
            ".bx .l{font-size:.55rem;color:rgba(196,181,253,.35);text-transform:uppercase;letter-spacing:.5px;margin-top:3px;font-weight:500}"
            ".sx{background:linear-gradient(135deg,rgba(99,102,241,.04),rgba(139,92,246,.02));border:1px solid rgba(99,102,241,.08);border-radius:14px;padding:14px;margin-bottom:10px}"
            ".sx h3{color:rgba(196,181,253,.4);font-size:.55rem;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:10px;font-weight:600}"
            ".rw{display:flex;justify-content:space-between;padding:3px 0;font-size:.75rem}"
            ".rw .lb{color:rgba(196,181,253,.3)}"
            ".rw .vl{font-weight:700;color:#e0e7ff}"
            ...
            ".f.ok{background:linear-gradient(90deg,#34d399,#6ee7b7)}"
            ".f.er{background:linear-gradient(90deg,#f87171,#fb923c)}"
            ".f.wn{background:linear-gradient(90deg,#fbbf24,#f59e0b)}"
            ".at{background:linear-gradient(135deg,rgba(248,113,113,.06),rgba(251,146,60,.03));border:1px solid rgba(248,113,113,.1);border-radius:14px;padding:14px;margin-bottom:10px}"
            ".at h3{color:#fca5a5;font-size:.55rem;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:8px;font-weight:600}"
            ".bt{display:block;width:100%%;padding:13px;margin-top:10px;border:none;border-radius:12px;font-size:.8rem;font-weight:600;cursor:pointer;text-decoration:none;text-align:center;transition:all .2s}"
            ".bt:active{transform:scale(.97)}"
            ".bp{background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;box-shadow:0 4px 15px rgba(99,102,241,.3)}"
            ".bo{background:rgba(99,102,241,.08);border:1px solid rgba(99,102,241,.15);color:#a5b4fc}"
            ".br{background:rgba(248,113,113,.08);border:1px solid rgba(248,113,113,.15);color:#fca5a5}"
            ...
            "</style></head><body>"
            "<div class=ov id=ov onclick='fd()'></div>"
            "<div class=dr id=dr>"
            "<div class=dh><h2>&#128737; Agente WiFi</h2>"
            "<p><span class='dot on' id=ds></span><span id=ip>-</span></p></div>"
            "<div class=dn>"
            "<a class='di act' href=/>&#127968; Painel</a>"
            "<a class='di' href=/relatorio>&#128202; Relatorios</a>"
            "<a class='di' href=/scan>&#128269; Redes WiFi</a>"
            "<a class='di' href=/reset_wifi>&#128260; Mudar Rede</a>"
            "</div>"
            "<div class=df><p>ESP32-C6 v1.0</p></div>"
            "</div>"
            "<header class=tb>"
            "<button class=hm onclick='ab()'>&#9776;</button>"
            "<div class=ti><small><span class='dot on' id=dt></span> Online</small>Agente WiFi</div>"
            "</header>"
            "<div class=mn>"
            "<div class=g>"
            "<div class=bx><div class=v id=v1>0</div><div class=l>Quadros</div></div>"
            "<div class=bx><div class=v id=v2 style='background:linear-gradient(135deg,#f87171,#fb923c);-webkit-background-clip:text;-webkit-text-fill-color:transparent'>0</div><div class=l>Deauth</div></div>"
            "<div class=bx><div class=v id=v3 style='background:linear-gradient(135deg,#fbbf24,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent'>0</div><div class=l>Disassoc</div></div>"
            "<div class=bx><div class=v id=v4 style='background:linear-gradient(135deg,#f87171,#dc2626);-webkit-background-clip:text;-webkit-text-fill-color:transparent'>0</div><div class=l>Atacantes</div></div>"
            "</div>"
            "<div class=sx><h3>&#128225; Trafego</h3>"
            "<div class=rw><span class=lb>Por segundo</span><span class=vl id=v5>0</span></div>"
            "<div class=bar><div class='f ok' id=b1 style=width:0></div></div>"
            "<div class=rw style=margin-top:8px><span class=lb>Canal</span><span class=vl id=v6>-</span></div>"
            "</div>"
            "<div class=at><h3>&#9888; Atacantes</h3>"
            "<div id=al style=color:rgba(196,181,253,.25);font-size:.7rem>Nenhum detectado</div></div>"
            "<a class='bt bp' href=/relatorio>&#128202; Ver Relatorio Completo</a>"
            "<a class='bt bo' href=/scan>&#128269; Ver Redes WiFi</a>"
            "<a class='bt br' href=/reset_wifi>&#128260; Mudar Rede WiFi</a>"
            "</div>"
            "<script>"
            "var dr=document.getElementById('dr'),ov=document.getElementById('ov');"
            "function ab(){dr.classList.add('open');ov.classList.add('on')}"
            "function fd(){dr.classList.remove('open');ov.classList.remove('on')}"
            "dr.addEventListener('touchstart',function(e){if(e.target===dr)fd()});"
            "var la=0;"
            "async function u(){"
            "try{"
            ...
            "b.className='f '+(p>80?'er':p>40?'wn':'ok');"
...
static esp_err_t relatorio_handler(httpd_req_t *req) {
    ...
    static const char html[] =
        ...
        ".dh{padding:24px 18px 16px;border-bottom:1px solid rgba(99,102,241,.1)}"
        ".dh h2{font-size:1.05rem;font-weight:700;color:#e0e7ff}"
        ".dh p{font-size:.6rem;color:rgba(196,181,253,.4);margin-top:4px}"
        ".dn{flex:1;padding:10px 0}"
        ".di{display:flex;align-items:center;gap:14px;padding:13px 18px;text-decoration:none;color:rgba(196,181,253,.45);font-size:.85rem;font-weight:500;border-radius:0 24px 24px 0;margin-right:12px;transition:all .2s}"
        ".di:active{background:rgba(99,102,241,.08);color:#c4b5fd}"
        ".di.act{color:#818cf8;background:linear-gradient(90deg,rgba(99,102,241,.12),rgba(99,102,241,.03));border-left:3px solid #6366f1}"
        ...
        ".df p{font-size:.5rem;color:rgba(196,181,253,.2);text-align:center}"
        ".mn{padding:16px;padding-top:72px}"
        ".s{background:linear-gradient(135deg,rgba(99,102,241,.06),rgba(139,92,246,.04));border:1px solid rgba(99,102,241,.1);border-radius:14px;padding:14px;margin-bottom:10px}"
        ".s h3{color:rgba(196,181,253,.5);font-size:.55rem;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:8px;font-weight:600}"
        ".r{display:flex;justify-content:space-between;padding:4px 0;font-size:.75rem}"
        ".r .l{color:rgba(196,181,253,.3)} .r .v{font-weight:700;color:#e0e7ff}"
        ".ok{color:#34d399}" ".er{color:#f87171}" ".wn{color:#fbbf24}"
        ".mc{font-family:monospace;color:#c4b5fd}"
        "</style></head><body>"
        "<div class=ov id=ov onclick='fd()'></div>"
        "<div class=dr id=dr>"
        "<div class=dh><h2>&#128737; Agente WiFi</h2><p>Relatorios de Ataques</p></div>"
        "<div class=dn>"
        "<a class='di' href=/>&#127968; Painel</a>"
        "<a class='di act' href=/relatorio>&#128202; Relatorios</a>"
        "<a class='di' href=/scan>&#128269; Redes WiFi</a>"
        "<a class='di' href=/reset_wifi>&#128260; Mudar Rede</a>"
        "</div>"
        "<div class=df><p>ESP32-C6 v1.0</p></div>"
        ...
        "<header class=tb>"
        "<button class=hm onclick='ab()'>&#9776;</button>"
        "<div class=ti><small>Relatorios</small>&#128202; Historico de Ataques</div>"
        "</header>"
        "<div class=mn>"
        "<div class=s><h3>Resumo</h3>"
        "<div class=r><span class=l>Total de Quadros</span><span class=v id=tr>0</span></div>"
        "<div class=r><span class=l>Deauth Detectados</span><span class=v id=td style='color:#f87171'>0</span></div>"
        "<div class=r><span class=l>Disassoc Detectados</span><span class=v id=tu style='color:#fbbf24'>0</span></div>"
        "<div class=r><span class=l>Total Atacantes</span><span class=v id=ta style='color:#f87171'>0</span></div>"
        "<div class=r><span class=l>Canal Atual</span><span class=v id=tc>-</span></div>"
        "</div>"
        "<div class=s><h3>&#128196; Historico de Ataques</h3>"
        ...
        "h='<table style=width:100%%;border-collapse:collapse><tr><th style=color:rgba(196,181,253,.35);font-size:.6rem;text-transform:uppercase;text-align:left;padding:6px 4px;border-bottom:1px solid rgba(99,102,241,.1)>MAC</th><th style=color:rgba(196,181,253,.35);font-size:.6rem;text-transform:uppercase;text-align:left;padding:6px 4px;border-bottom:1px solid rgba(99,102,241,.1)>Tipo</th><th style=color:rgba(196,181,253,.35);font-size:.6rem;text-transform:uppercase;text-align:left;padding:6px 4px;border-bottom:1px solid rgba(99,102,241,.1)>Qtd</th><th style=color:rgba(196,181,253,.35);font-size:.6rem;text-transform:uppercase;text-align:left;padding:6px 4px;border-bottom:1px solid rgba(99,102,241,.1)>Canal</th></tr>';"
        "for(var i=0;i<d.log.length;i++){"
        "var a=d.log[i];"
        "h+='<tr><td class=mc style=padding:6px 4px;border-bottom:1px solid rgba(99,102,241,.05)>'+a.mac+'</td><td style=padding:6px 4px;border-bottom:1px solid rgba(99,102,241,.05)>'+(a.type=='deauth'?'<span style=color:#f87171>Deauth</span>':'<span style=color:#fbbf24>Disassoc</span>')+'</td><td style=padding:6px 4px;border-bottom:1px solid rgba(99,102,241,.05);color:#e0e7ff>'+a.count+'</td><td style=padding:6px 4px;border-bottom:1px solid rgba(99,102,241,.05);color:#e0e7ff>'+a.channel+'</td></tr>';"
        "}h+='</table>';"
